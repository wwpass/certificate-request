...
function composeAttribute(companyData){
    ...
    attributesData.push(
      new asn1js.Sequence({
        value: [
          ...
          new asn1js.Set({ value: [new asn1js.Utf8String({ value: companyData[OID_UNSTRUCTURED_NAME] })] })
        ]
      })
    );
  }
      ...
      new asn1js.Sequence({
        value: [
          ...
          new asn1js.Set({ value:
            companyData[OID_EXTENSION_REQUEST].map(entry => {
  ...
  if (attributesData.length > 0) {
    let attributesBuf = new Uint8Array([0xa0, 0x00]); //empty constructed context-specific class with tag 0
    let attributes = asn1js.fromBER(attributesBuf.buffer).result;
...
function composeName(companyData){
  ...
  const companyDataKeys = Object.keys(companyData).filter(oid => {
    return oid in attrubuteType;
  });
  const attributesArr = companyDataKeys.map(oid => {
